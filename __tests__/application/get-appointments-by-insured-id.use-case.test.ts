...
describe("GetAppointmentsByInsuredIdUseCase", () => {
  ...
  const getAppointmentsByInsuredIdUseCase =
    new GetAppointmentsByInsuredIdUseCase(mockDynamoDBRepository);
  ...
  it("should retrieve appointments by insured ID", async () => {
    ...
    // Assert
    expect(mockDynamoDBRepository.findAllByInsuredId).toHaveBeenCalledWith(
      insuredId
    );
  ...
  it("should throw an error if repository throws an error", async () => {
    ...
    // Act & Assert
    await expect(
      getAppointmentsByInsuredIdUseCase.execute(insuredId)
