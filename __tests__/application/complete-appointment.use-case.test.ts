...
describe('CompleteAppointmentUseCase', () => {
  ...
  const completeAppointmentUseCase = new CompleteAppointmentUseCase(mockAppointmentRepository);
  ...
  it('should update appointment status to COMPLETED', async () => {
    ...
    mockAppointmentRepository.processAppointment.mockResolvedValue(completedAppointment);
  ...
  it('should throw an error if repository update fails', async () => {
    ...
    // Act & Assert
    await expect(completeAppointmentUseCase.execute(appointment)).rejects.toThrow('Database error');
