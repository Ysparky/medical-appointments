...
describe('ProcessAppointmentUseCase', () => {
  ...
  const processAppointmentUseCase = new ProcessAppointmentUseCase(mockEventBusAdapter);
  ...
  it('should publish processed appointment to event bus', async () => {
    ...
    // Assert
    expect(mockEventBusAdapter.publishProcessedAppointment).toHaveBeenCalledWith(appointment);
  });
  ...
  it('should throw an error if event bus publish fails', async () => {
    ...
    // Act & Assert
    await expect(processAppointmentUseCase.execute(appointment)).rejects.toThrow('Event bus error');
    expect(mockEventBusAdapter.publishProcessedAppointment).toHaveBeenCalledWith(appointment);
  });
});
